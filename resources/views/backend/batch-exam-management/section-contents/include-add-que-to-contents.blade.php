...
<form action="{{ route('assign-question-to-batch-exam-content') }}" method="post" id="" enctype="multipart/form-data">
    @csrf
...
{{--                                @include('backend.batch-exam-management.section-contents.include-question-topic-loop', ['questionTopic' => $questionTopic, 'child' => 1])--}}
{{--                            @endif--}}
{{--                        @endif--}}
{{--                    @endforeach--}}
{{--                </select>--}}
                ...
                <div class="" id="">
                    @foreach($questionTopics as $key => $questionTopic)
                        <div class="parent-div">
                            ...
                            @if(!empty($questionTopic))
                                @if(count($questionTopic->questionTopics) > 0)
                                    @include('backend.batch-exam-management.section-contents.question-topic-loop', ['questionTopic' => $questionTopic, 'child' => 15])
                                @endif
                            @endif
                        </div>
                    @endforeach
